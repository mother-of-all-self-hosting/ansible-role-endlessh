...
#
# Tags look like `v<Endlessh version>-<release>`:
#
# - if defaults/main.yml points at an Endlessh version that has never been
#   released, the release counter restarts at 0 (`v2026.0730.0-0`)
# - otherwise the counter is incremented (`v2026.0730.0-1`), but only if
...
version="$(sed -nE 's|^endlessh_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Endlessh version from $defaults_path"
	exit 1
fi

# The version values carry no leading `v` (e.g. `2026.0730.0`) while the tags do
# (`v2026.0730.0-0`). Stripping any `v` before prepending one keeps this correct
